...
def kpt_overlaps(gt_keypoints, gt_bboxes, gt_bboxes_ignore, templates, mode='iou'):
    ...
    if True:
        ...
        overlaps = []
        for k in range(gt_keypoints.size(0)):
            gt_keypoint = gt_keypoints[k]
            gt_bbox = gt_bboxes[k]
            bbox_area = (gt_bbox[2]-gt_bbox[0]+1)*(gt_bbox[3]-gt_bbox[1]+1)
            overlaps.append(
                oks_iou_tensor(gt_keypoint.view(-1), templates.view(templates.shape[0], -1), bbox_area, bbox_area))
...
class MaxOksIoUAssignerGtbbox(BaseAssigner):
    ...
    def assign(self, templates, templates_scales, anchor_infos, gt_keypoints, gt_bboxes, gt_bboxes_ignore, gt_labels=None):
        """Assign gt to bboxes.
        ...
        # bboxes = bboxes[:, :4]
        overlaps = kpt_overlaps(gt_keypoints, gt_bboxes, gt_bboxes_ignore, templates)
