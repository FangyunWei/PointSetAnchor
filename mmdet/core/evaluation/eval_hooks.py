...
class CocoPoseDistEvalmAPHook(DistEvalHook):
    ...
    def evaluate(self, runner, results):
        ...
        if True:
            ...
            out_str_bbox = ['box_mAP', 'box_mAP_50', 'box_mAP_75', 'box_mAP_s', 'box_mAP_m', 'box_mAP_l']
            # out_str_bbox = []
            ...
            for i in range(len(out_str_bbox)):
                key = out_str_bbox[i]
                val = float('{:.3f}'.format(cocoEval.stats[i]))
                runner.log_buffer.output[key] = val
        ...
        if True:
            cocoGt = self.dataset.coco
            cocoDt = cocoGt.loadRes(result_files['keypoints'])
            imgIds = cocoGt.getImgIds()
            iou_type = 'keypoints'
            cocoEval = COCOeval(cocoGt, cocoDt, iou_type)
            cocoEval.params.imgIds = imgIds
            ...
            out_str_kpts = ['kpt_mAP', 'kpt_mAP_50', 'kpt_mAP_75', 'kpt_mAP_m', 'kpt_mAP_l']
            # out_str_kpts = []
            ...
            for i in range(len(out_str_kpts)):
                key = out_str_kpts[i]
                val = float('{:.3f}'.format(cocoEval.stats[i]))
                runner.log_buffer.output[key] = val
        ...
        os.remove(result_files['keypoints'])
