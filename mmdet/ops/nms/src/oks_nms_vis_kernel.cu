...
__device__ inline float devIoU(float const * const kpts_g, float const * const kpts_d, const float vis_thresh, float const * const sigmas) {
    ...
    int count = 0;
    for(int i = 0;i < KPTS_LEN; ++i){
            if((kpts_g[i + 2 * KPTS_LEN + 2] > vis_thresh) && (kpts_d[i + 2 * KPTS_LEN + 2] > vis_thresh)){
                e_sum += exp(-1 * e[i]);
                count+=0;
            }
    }
...
__global__ void oks_nms_kernel(const int n_kpts, const float overlap_thresh, const float vis_thresh,
  ...
  __shared__ float block_kpts[threadsPerBlock * (KPTS_LEN * 3 + 2)];
  if (threadIdx.x < col_size) {
      for(int i = 0; i < (KPTS_LEN * 3 + 2); ++i){
        block_kpts[threadIdx.x * (KPTS_LEN * 3 + 2) + i] =
            dev_kpts[(threadsPerBlock * col_start + threadIdx.x) * (KPTS_LEN * 3 + 2) + i];
      }
  }
  __syncthreads();
  ...
  if (threadIdx.x < row_size) {
    ...
    const float *cur_kpt = dev_kpts + cur_box_idx * (KPTS_LEN * 3 + 2);
    int i = 0;
    ...
    for (i = start; i < col_size; i++) {
      if (devIoU(cur_kpt, block_kpts + i * (KPTS_LEN * 3 + 2), vis_thresh, sigmas) > overlap_thresh) {
        t |= 1ULL << i;
      }
    }
